...
const RegisterCard = () => {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm({ resolver: yupResolver(schemaShape) });
  ...
  const submit = handleSubmit(async (data) => {
    ...
    try {
      const res = await axios.post(`${mainLink}/register`, {
        firstname,
        lastname: surname,
        email,
        password,
      ...
      <RegisterCardWrapper>
        ...
        <CheckAndText>
          ...
          <CheckTxt>
            ...
            <code style={{ color: '#7b69dd' }}> Privacy Policy</code> &{' '}
            <code style={{ color: '#7b69dd' }}>Terms and Conditions</code>
          </CheckTxt>
        </CheckAndText>
        ...
        {checktoggle ? (
          ...
          <Button1 bg="lightgray">Create an Account</Button1>
        )}
        <RouteToSignUp>
