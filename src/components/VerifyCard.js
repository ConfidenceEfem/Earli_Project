...
const verifyEmailAndLogin = async ()=>{
    ...
    await axios.post(`${mainLink}/verify`, {firstname,lastname,email,password,otp: num+num1+num2+num3+num4+num5})
  ...
  return (
    <Container>
      ...
      <VerifySub>Fill in the code we sent to <span>{email}</span></VerifySub>
      <VerifyChange to="/signup">Change Email {otpHold}</VerifyChange>
      </VerifyText>
      <InputsAndButton>
      <InputHolder>
      <Inputs
      ...
      {counter===0 || counter<0? <Resend>Code Expired Already</Resend> : <Resend>Code expires in <span>{counter}secs</span></Resend>}
      {counter===0 || counter<0?
      
      <Button bg="lightgray">Continue</Button>:<Button tr
      bg="#7B69DD"
      onClick={()=>{
        verifyEmailAndLogin()
